...
public class ArcaneCraftingResultSlot extends Slot {
    ...
    public void onTake(Player pPlayer, ItemStack pStack) {
        this.checkTakeAchievements(pStack);
        NonNullList<ItemStack> remains = pPlayer.level.getRecipeManager().getRemainingItemsFor(ChaumtraftRecipes.ARCANE_CRAFTING.type(), this.craftSlots, pPlayer.level);
        ArcaneCraftingRecipe recipe = (ArcaneCraftingRecipe)((ResultContainer)container).getRecipeUsed();
        for(int i = 0; i < remains.size(); ++i) {
            ...
            if(!currentSlot.isEmpty()) {
                if(i < 6) {
                    ...
                    if(recipe != null && recipe.getCrystals().containsKey(primal)) {
                        Chaumtraft.info("Removing %s crystals. [%d]", primal.getSerializedName(), recipe.getCrystals().get(primal));
            ...
            if(!slotRemains.isEmpty()) {
                ...
                } else if(ItemStack.isSame(currentSlot, slotRemains) && ItemStack.tagMatches(currentSlot, slotRemains)) {
