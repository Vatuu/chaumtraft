...
public class ArcaneWorkbenchMenu extends AbstractContainerMenu {
    ...
    private void craftingComponentsUpdated(Level pLevel, BlockPos pos, Player pPlayer) {
        if (!pLevel.isClientSide) {
            ...
            Optional<ArcaneCraftingRecipe> optional = CraftingUtils.findArcaneCraftingRecipe(pLevel, this.craftingSlots, cap);
            if (optional.isPresent()) {
                ArcaneCraftingRecipe recipe = optional.get();
                data.set(DATA_ACTIVE_CRYSTALS, MathUtils.create8Bitfield(recipe.getCrystals().keySet()));
                data.set(DATA_REQUIRED_VIS, recipe.getVisAmount());
    ...
    private void populateCrystalSlots() {
        addSlot(new PredicateSlot(this.craftingSlots, SLOT_CHAOS, 57, 81, i -> AbstractAspectItem.hasAspect(i, ChaumtraftIDs.Aspects.CHAOS)));
        addSlot(new PredicateSlot(this.craftingSlots, SLOT_WATER, 105, 1, i -> AbstractAspectItem.hasAspect(i, ChaumtraftIDs.Aspects.WATER)));
        addSlot(new PredicateSlot(this.craftingSlots, SLOT_ORDER, 105, 59, i -> AbstractAspectItem.hasAspect(i, ChaumtraftIDs.Aspects.ORDER)));
        addSlot(new PredicateSlot(this.craftingSlots, SLOT_AIR, 57, -21, i -> AbstractAspectItem.hasAspect(i, ChaumtraftIDs.Aspects.AIR)));
        addSlot(new PredicateSlot(this.craftingSlots, SLOT_FIRE, 10, 1, i -> AbstractAspectItem.hasAspect(i, ChaumtraftIDs.Aspects.FIRE)));
        addSlot(new PredicateSlot(this.craftingSlots, SLOT_EARTH, 10, 59, i -> AbstractAspectItem.hasAspect(i, ChaumtraftIDs.Aspects.EARTH)));
    }
}
